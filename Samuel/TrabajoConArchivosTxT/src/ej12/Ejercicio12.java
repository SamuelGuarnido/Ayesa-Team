...
public class Ejercicio12 {
	...
	static void busca(String nombre) {
		ArrayList<String> same = new ArrayList<>();
		String name = "";
		
		String[] subNombres = txt.split(" ");
		for(int i = 0; i < subNombres.length; i++) {
			if(subNombres[i].toLowerCase().contains(nombre.toLowerCase())) {
				same.add(subNombres[i]);
			}
